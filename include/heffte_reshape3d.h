...
public:
    ...
    //template<typename b, template<typename d> class p> friend std::unique_ptr<reshape3d_alltoallv<b, p>>
    //make_reshape3d_osc_alltoallv(std::vector<box3d> const&, std::vector<box3d> const&, MPI_Comm const);
    ...
    // For persistent A2AV
    void Init_myinfo(myinfo* P_info)
    {
        P_info = (myinfo*)malloc(sizeof(myinfo));
        P_info->recv_bytes = NULL;
        P_info->P_inited = 0;
        printf("Init_myinfo, P_info %p, P_info->recv_bytes %p, P_info->win %p, P_info->P_inited %d\n", P_info, P_info->recv_bytes, &(P_info->win), P_info->P_inited);
    }
